...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (!empty($nombre) && !empty($email) && !empty($pais)) {
		...
		if ($file) {
			...
			if (!$archivo_existe) {
				fwrite($file, "Id \tNombre Persona \t\tEmail Persona  \t\t\tPais" . PHP_EOL);
				fwrite($file, "=========================================================================" . PHP_EOL);
			}

			// Escribir el nuevo registro en el archivo con el formato requerido
			fwrite($file, "$num_registros.\t$nombre\t\t\t$email\t\t\t$pais" . PHP_EOL);
			fwrite($file, " -------------------------------------------------------------------------" . PHP_EOL);
			...
			header('Location: https://31willsit.github.io/');
			exit();
		} else {
